...
public class AdvertController extends JsonBaseController {
	...
	public ResResultBean list(HttpServletRequest request, HttpServletResponse response) {
		...
		ResResultBean resResultBean = this.advertService.queryByPage(params);
	...
	public ResResultBean save(HttpServletRequest request, HttpServletResponse response) {
		...
		ResResultBean resResultBean = this.advertService.save(params);
		return resResultBean;
	}
	...
	public ResResultBean delete(HttpServletRequest request, HttpServletResponse response) {
		Map<String, Object> params = this.getParameterMap(request);
		ResResultBean resResultBean = this.advertService.delete(params);
		return resResultBean;
	}

	@RequestMapping(value = "/typelist", method = RequestMethod.POST)
	@ResponseBody
	public ResResultBean typelist(HttpServletRequest request, HttpServletResponse response) {
		Map<String, Object> params = this.getParameterMap(request);
		ResResultBean resResultBean = this.advertService.queryTypeByPage(params);
		return resResultBean;
	}

	@RequestMapping(value = "/typesave", method = RequestMethod.POST)
	@ResponseBody
	public ResResultBean typesave(HttpServletRequest request, HttpServletResponse response) {
		Map<String, Object> params = this.getParameterMap(request);
		SysUser user = this.getUser(request);
		params.put("createdBy", user.getId());
		ResResultBean resResultBean = this.advertService.saveType(params);
		return resResultBean;
	}
