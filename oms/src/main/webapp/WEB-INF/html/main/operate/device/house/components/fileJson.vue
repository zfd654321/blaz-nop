  ...
  <el-drawer :title="jsonData.configList.group.cname" :visible.sync="innerdrawer" ref="innerdrawer" :before-close="handleClose" size="1280" append-to-body>
    ...
    <el-tabs tab-position="left" v-model="groupChecked" @tab-click="listChecked='0'" style="height:800">
      <el-tab-pane v-for="(group,gindex) in jsonData.defaultList.group" :key="gindex" :name="gindex+''">
        <span slot="label">
          <el-tag :type="gindex==groupChecked?'':'info'" closable @close="removeGroup(gindex)" style="width:330;overflow:hidden; white-space:nowrap; text-overflow:ellipsis;">
            <div style="width:290;display: inline-block;text-align: left;">{{group.groupname[0]}}</div>
          </el-tag>
        </span>
        ...
        <el-form label-position="top">
          <el-form-item :label="item.cname" v-for="item of jsonData.configList.group.file" :key="item.name">
            <el-input v-for="(gitem,giindex) in group[item.name]" :readonly="item.type!='text'" :key="giindex" v-model="group[item.name][giindex]" size="mini" style="width:350">
              <el-button type="success" plain v-if="item.type!='text'" slot="append" icon="el-icon-picture-outline" @click="checkGroupResource(item.name,giindex,item.type)"></el-button>
            </el-input>
          </el-form-item>
          <el-form-item :label="jsonData.configList.group.list.cname">
            <el-tabs tab-position="left" v-model="listChecked" style="height:550">
              <el-tab-pane v-for="(list,lindex) in group.list" :key="lindex" :name="lindex+''">
                <span slot="label">
                  <el-tag :type="lindex==listChecked?'':'info'" closable @close="removeList(lindex)" style="width:330;overflow:hidden; white-space:nowrap; text-overflow:ellipsis;">
                    <div style="width:290;display: inline-block;text-align: left;">{{list.listname[0]}}</div>
                  </el-tag>
                </span>
                <el-form label-position="top" style="height:550;overflow-y:auto">
                  <el-form-item :label="item.cname" v-for="(item,findex) of jsonData.configList.group.list.file" :key="item.name">
                    <el-input v-for="(litem,liindex) in list[item.name]" :readonly="item.type!='text'" :key="liindex" v-model="list[item.name][liindex]" size="mini" style="width:350;margin-top:15">
                      <el-button type="success" plain v-if="item.type!='text'" slot="append" icon="el-icon-picture-outline" @click="checkResource(item.name,liindex,item.type)"></el-button>
                      <el-button type="success" plain v-if="list[item.name].length>jsonData.configList.group.list.file[findex].minLength" slot="append" icon="el-icon-delete" @click="removeResource(item.name,liindex)"></el-button>
                    </el-input>
                    <div>
                      <el-button size="mini" @click="addResource(item.name)" style="margin-top:15;margin-left: 295" v-if="jsonData.configList.group.list.file[findex].maxLength>list[item.name].length">
                        添加
                      </el-button>
                    </div>
                  </el-form-item>
              ...
              <el-tab-pane disabled>
                <span slot="label">
                  <el-button size="mini" @click="addList()" style="margin-left:185" v-if="jsonData.configList.group.list.maxLength>group.list.length">
                    添加
                  </el-button>
                </span>
              </el-tab-pane>
      ...
      <el-tab-pane disabled>
        <span slot="label">
          <el-button size="mini" @click="addGroup()" style="margin-left:185" v-if="jsonData.configList.group.maxLength>jsonData.defaultList.group.length">
            添加
          </el-button>
        </span>
      </el-tab-pane>
